...
function ActionButtons({ onScanClick, onDemoClick, onDailyIntakeClick, onAlertsClick, isScanning, nutritionData }) {
    ...
    const handleDemoButtonClick = (e) => {
        console.log('ActionButtons: TRY DEMO button clicked.');
        e.preventDefault();
        e.stopPropagation();
        if (fileInputRef.current) {
            console.log('ActionButtons: Triggering file input click.');
            fileInputRef.current.click();
        } else {
            console.error('ActionButtons: fileInputRef.current is null!');
        }
    };

    const handleFileChange = (event) => {
        const file = event.target.files?.[0];
        console.log('ActionButtons: File input change detected. File:', file);
        if (file) {
            onDemoClick(file);
